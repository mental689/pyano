...
function ui_loadprevious(job, objectui)
{
    ...
    server_request("getboxesforjob", {'id': job.jobid}, function(data) {
        overlay.remove();
        note.remove();
        console.log('Found ' + data.result.length + ' boxes.');
...
function ui_submit(job, tracks)
{
    ...
    {
        server_post("validatejob", [], {'csrfmiddlewaretoken': $.cookie("csrftoken"), 'id': job.jobid, 'tracks': tracks.serialize()},
            function(valid) {
                if (valid)
                {
    ...
    {
        server_request("respawnjob", {'id': job.jobid}, function() {
            callback();
        });
    }
    ...
    {
        server_post("savejob", [], {'csrfmiddlewaretoken': $.cookie("csrftoken"), 'id': job.jobid, 'tracks': tracks.serialize()}, function(data) {
                callback()
            });
    }
