(function(global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ?
      factory(exports, require('d3-axis'), require('d3-array'),
          require('d3-time-format'), require('d3-time'), require('d3-scale'),
          require('d3-selection'), require('d3-zoom')) :
      typeof define === 'function' && define.amd ?
          ...
          (factory((global.d3 = global.d3 || {}), global.d3, global.d3,
              global.d3, global.d3, global.d3, global.d3, global.d3));
}(this, function(exports, d3Axis, d3Array, d3TimeFormat, d3Time, d3Scale,
                 d3Selection, d3Zoom) {
  'use strict';
  ...
  var timelines = function() {
    ...
    var appendTimeAxisTick = function(g, xAxis, maxStack) {
      g.append('g').
          ...
          call(xAxis.tickFormat('').
              tickSize(
                  -(margin.top + (itemHeight + itemMargin) * (maxStack - 1) +
                      3), 0, 0));
    };
    ...
    var appendBackgroundBar = function(yAxisMapping, index, g, data, datum) {
      ...
      g.selectAll('svg').
          ...
          attr('width', fullLengthBackgrounds ?
              width :
              (width - margin.right - margin.left)).
          attr('y', greenbarYAxis).
          attr('height', itemHeight).
          attr('fill', backgroundColor instanceof Function ?
    ...
    function timelines(gParent) {
      ...
      var scaleFactor = (1 / (ending - beginning)) *
      ...
      if (tickFormat.tickValues !== null) {
        ...
        xAxis.tickArguments(tickFormat.numTicks ||
            [tickFormat.tickTime, tickFormat.tickInterval]);
      }
      ...
      g.each(function(d, i) {
        ...
        d.forEach(function(datum, index) {
          ...
          view.selectAll('svg').
              ...
              enter().
              append(function(d, i) {
                return document.createElementNS(d3Selection.namespaces.svg,
              ...
              on('click', function(d, i) {
                ...
                click(d, index, datum, selectedLabel, selectedRect,
          ...
          if (rowSeparatorsColor) {
            var lineYAxis = (itemHeight + itemMargin / 2 + margin.top +
          ...
          if (typeof(datum.icon) !== 'undefined') {
            gParent.append('image').
                attr('class', 'timeline-label').
                attr('transform', 'translate(' + 0 + ',' +
                    (margin.top + (itemHeight + itemMargin) *
                        yAxisMapping[index]) + ')').
          ...
          function getStackPosition(d, i) {
            if (stacked) {
              return margin.top + (itemHeight + itemMargin) *
                  yAxisMapping[index];
            }
            return margin.top;
          }
          ...
          function getStackTextPosition(d, i) {
            if (stacked) {
              return margin.top + (itemHeight + itemMargin) *
                  yAxisMapping[index] + itemHeight * 0.75;
            }
      ...
      if (width > gParentSize.width) { // only if the scrolling should be allowed
        var move = function() {
          g.select('.view').
              attr('transform', 'translate(' + d3Selection.event.transform.x +
                  ',0)'
                  + 'scale(' + d3Selection.event.transform.k + ' 1)');

          g.selectAll('.timeline-xAxis').attr('transform', function(d) {
            return 'translate(' + d3Selection.event.transform.x + ', ' +
                timeAxisYPosition + ')'
                + 'scale(' + d3Selection.event.transform.k + ' 1)';
          });
      ...
      var zoom = d3Zoom.zoom().scaleExtent([0, maxZoom]) // max zoom defaults to 5
      ...
      function getXTextPos(d, i, text, style) {
        ...
        if (text && style) {
          ...
          var xPosition = margin.left +
              ((d.starting_time - beginning) * scaleFactor) + width -
              textLength;
          return xPosition;
        } else {
    ...
    timelines.display = function(displayType) {
      if (!arguments.length ||
          (DISPLAY_TYPES.indexOf(displayType) == -1)) return display;
      display = displayType;
      return timelines;
    };
